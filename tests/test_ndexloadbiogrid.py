...
class TestNdexbiogridloader(unittest.TestCase):
    ...
    def test_parse_arguments(self):
        ...
        res = ndexloadbiogrid._parse_arguments('hi', [])

        self.assertEqual(res.profile, 'ndexbiogridloader')
        ...
        someargs = ['-vv','--conf', 'foo', '--logconf', 'hi',
                    '--profile', 'myprofy']
        res = ndexloadbiogrid._parse_arguments('hi', someargs)
    ...
    def test_setup_logging(self):
        """ Tests logging setup"""
        try:
            ndexloadbiogrid._setup_logging(None)
        ...
        res = ndexloadbiogrid._parse_arguments('hi', [])
        ndexloadbiogrid._setup_logging(res)
...
format=%(asctime)s %(name)-12s %(levelname)-8s %(message)s""")
            ...
            res = ndexloadbiogrid._parse_arguments('hi', ['--logconf',
                                                                       logfile])
            ndexloadbiogrid._setup_logging(res)
    ...
    def test_main(self):
        ...
        try:
            ...
            res = ndexloadbiogrid.main(['myprog.py', '--conf',
                                                     confile, '--profile',
                                                     'hi'])
